 ...
 * @example
 * ```ts
 * import { getUint16BE } from "@quentinadam/uint8array-extension";
 * import assert from "@quentinadam/assert";
 *
 * const bytes = new Uint8Array([3, 232]);
 * assert(getUint16BE(bytes, 0) === 1000);
 * ```
 *
 ...
 */
export function getUint16BE(target: Uint8Array, offset: number): number {
