import { fromUint32 } from './fromUint32.ts';

/**
 * Creates a 4-byte big-endian Uint8Array from an unsigned 32-bit integer.
 *
 * @example
 * ```ts
 * import { fromUint32BE, equals } from "@quentinadam/uint8array-extension";
 * import assert from "@quentinadam/assert";
 *
 * assert(equals(fromUint32BE(1000), new Uint8Array([0, 0, 3, 232])));
 * assert(equals(fromUint32BE(4294967295), new Uint8Array([255, 255, 255, 255])));
 * ```
 *
 * @param value The unsigned 32-bit integer value (0 to 4294967295).
 * @returns A 4-byte big-endian Uint8Array representing the value.
 * @throws If value is out of range for a 32-bit unsigned integer.
 */
export function fromUint32BE(value: number | bigint): Uint8Array<ArrayBuffer> {
  return fromUint32(value, false);
}
