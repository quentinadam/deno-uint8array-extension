 ...
 * @example
 * ```ts
 * import { getInt16LE } from "@quentinadam/uint8array-extension";
 * import assert from "@quentinadam/assert";
 *
 * const bytes = new Uint8Array([232, 3]);
 * assert(getInt16LE(bytes, 0) === 1000);
 * ```
 *
 ...
 */
export function getInt16LE(target: Uint8Array, offset: number): number {
